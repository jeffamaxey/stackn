...
class ModelCreate(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, user, project):
        ...
        # for the time being is hard-coded to jupyter-lab where usually models are trained
    ...
    def post(self, request, user, project):
        ...
        model_project = Project.objects.filter(Q(owner=request.user) | Q(
            authorized=request.user), status='active', slug=project).distinct().first()
...
def details_private(request, user, project, id):
    ...
    try:
        project = Project.objects.filter(Q(owner=request.user) | Q(
            authorized=request.user), status='active', slug=project_slug).first()
